...
class InferenceNet(nn.Module):
    def __init__(self, x_dim, z_dim, y_dim, n_gene, nonLinear):
        ...
        self.inference_qzyx_perturb = torch.nn.ModuleList([
    ...
    def qzxy_p(self, x, y):
        concat = torch.cat((x, y.unsqueeze(1).repeat(1, x.shape[1], 1)), dim=2)
        for i, layer in enumerate(self.inference_qzyx_perturb):
            concat = layer(concat)
        return concat
    ...
    def forward(self, x, p, adj, temperature=1.0):
        ...
        mu = mu * ~perturb_mask + mu_p * perturb_mask
        logvar = logvar_p * ~perturb_mask + logvar_p * perturb_mask
