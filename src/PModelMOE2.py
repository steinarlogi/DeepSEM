...
class InferenceNet(nn.Module):
    def __init__(self, x_dim, z_dim, y_dim, n_gene, nonLinear):
        ...
        self.inference_qzyx_first_layer = nn.Linear(x_dim + y_dim, z_dim)
        self.inference_qzyx_first_layer_perturb = nn.Linear(x_dim + y_dim, z_dim)
        self.z_dim = z_dim
    ...
    def qzxy(self, x, y, p):
        ...
        concat_normal = self.inference_qzyx_first_layer(concat)
        concat_p = self.inference_qzyx_first_layer_perturb(concat)
        p_mask = p[:, :, None].expand(-1, -1, self.z_dim) != 0
        concat = concat_normal * ~p_mask + concat_p * p_mask # Using different weights in the first layer depending on whether a gene is perturbed or not
...
class VAE_EAD(nn.Module):
    ...
    def forward(self, x, p, dropout_mask, temperature=1.0, opt=None):
        ...
        out_inf = self.inference(x, p, adj_A_t, temperature)
